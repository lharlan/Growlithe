...
class NoteCog(commands.Cog, name='Notes'):
    ...
    async def listnotes(self, ctx, member:discord.Member):
        ...
        embed.set_author(name="Notes for {}#{} | {}".format(member.display_name, member.discriminator, member.id), icon_url=member.display_avatar)
        with open("notes.json", "r") as f:
            notes = json.load(f)
        try:
    ...
    async def note(self, ctx, member:discord.Member, *, reason=""):
        ...
        if '@everyone' in reason or '@here' in reason:
            await ctx.send('This is not a valid reason!')
            return
        else:
