...
class WarnCog(commands.Cog, name = 'Warn'):
    ...
    async def listwarns(self, ctx, member:discord.Member):
        ...
        embed.set_author(name="Warns for {}#{} | {}".format(member.display_name, member.discriminator, member.id), icon_url=member.display_avatar)
        with open("warnings.json", "r") as f:
            warns = json.load(f)
        try:
    ...
    async def warn(self, ctx, member:discord.Member, *, reason=""):
        ...
        try:
            ...
            await ctx.send('Failed to DM user! I have been blocked.')
